...
import { PgwireQueryRouter } from "../src/pgwire/query-router.js";
import type { StdbTableRow } from "../src/shim/types.js";

class FakeStdbClient {
  async listDatabases(): Promise<string[]> {
    return ["fms-glm-control", "fms-glm-org-tt"];
  }
...
describe("pgwire query router", () => {
  const router = new PgwireQueryRouter(new FakeStdbClient() as never);
  ...
  it("returns FMS databases as user databases for pgAdmin browser listing", async () => {
    const result = await router.execute(
    ...
    expect(result.rows).toEqual([
      ["20000", "fms-glm-control"],
      ["20001", "fms-glm-org-tt"],
      ["5", "postgres"],
    ]);
  });
  ...
  it("passes through insert/update/delete with postgres command tags", async () => {
    await expect(
      router.execute("insert into public.orders (id) values (1)", "fms-glm-org-tt")
    ).resolves.toMatchObject({ fields: [], rows: [], commandTag: "INSERT 0 1" });

    await expect(
      router.execute("update public.orders set status = 'done' where id = 1", "fms-glm-org-tt")
    ).resolves.toMatchObject({ fields: [], rows: [], commandTag: "UPDATE 2" });

    await expect(
      router.execute("delete from public.orders where id = 1", "fms-glm-org-tt")
